...
export default function TransferPage() {
  ...
  const beginNewTransfer = useLiveStore(s => s.beginNewTransfer);
  ...
  if (!activeRecord) return <div>
    ...
    <div className="glass rounded-2xl p-6">
      <div className="grid gap-5 md:grid-cols-2">
        <label className="block text-sm text-slate-400">Sender wallet<input value={senderWallet} onChange={e => setField("senderWallet", e.target.value.trim())} placeholder="Enter sender TRON address" autoComplete="off" spellCheck={false} aria-describedby="sender-wallet-help" className="mono mt-2 w-full rounded-xl border border-line bg-black/20 px-3 py-3 text-xs text-white outline-none focus:border-cyan" /><span id="sender-wallet-help" className="mt-1 block text-xs text-slate-400">This address will appear as the sender on the receipt. Your connected wallet remains the sign-in account.</span></label>
        ...
        <label className="block text-sm text-slate-400">TRX / USD rate
          ...
          <span className={`mt-1 block text-xs ${rateState === "live" ? "text-mint" : "text-slate-500"}`}>{rateState === "loading" ? "Reading public price feed…" : rateState === "live" ? "Live public TRX price" : "Public feed unavailable — fallback live rate"}</span>
        </label>
  ...
  return <div>
    ...
    {status === "failed" && <div className="mt-5 rounded-2xl border border-red-400/25 bg-red-400/10 p-5"><div className="font-semibold text-red-200">Transfer live paused</div><button onClick={() => { setFailureMode(false); start(); }} className="mt-3 rounded-xl bg-red-300 px-4 py-2.5 text-sm font-bold text-red-950">Retry</button></div>}
    <div className="mt-5 flex flex-wrap gap-3"><button onClick={() => setFailureMode(!failureMode)} className="rounded-xl border border-line px-4 py-2.5 text-sm font-semibold text-slate-400">{failureMode ? "Failure live armed" : "Live Failure"}</button><button onClick={beginNewTransfer} className="rounded-xl border border-cyan/30 bg-cyan/10 px-4 py-2.5 text-sm font-semibold text-cyan transition hover:bg-cyan/20">Create another transfer</button></div>
  </div>;
}
