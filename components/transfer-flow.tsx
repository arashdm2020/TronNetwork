...
export function TransferFlow({ active }: { active: boolean }) {
  const record = useTransferStore(s => s.transferRecord);
  ...
  return <>
    ...
    {record && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-cyan/20 bg-cyan/5 px-4 py-3"><div><div className="eyebrow">Transfer document</div><div className="mono mt-1 text-xs text-cyan">{record.transferId}</div></div><Link href={transferDocumentPath(record)} className="rounded-lg border border-cyan/30 px-3 py-2 text-xs font-semibold text-cyan transition hover:bg-cyan/10">Open document</Link></div>}
  </>;
}
